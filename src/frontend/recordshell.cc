...
int main( int argc, char *argv[] )
{
    try {
        ...
        {
            auto pipe_dir = UnixDomainSocket::make_pair();

            ChildProcess mk("mkdir", [&](){
                drop_privileges();
                struct stat s;
                stat(directory.c_str(), &s);
                if (S_ISDIR(s.st_mode)) run({"/bin/rm", "-rf", directory});
                make_directory( directory );
                pipe_dir.first.write("x");
                return 0;
            });
            
            pipe_dir.second.read();
        }

        ChildProcess tcpdump("tcpdump", [&]() {
                        return ezexec({"/usr/sbin/tcpdump", "-i", "enp0s3", "tcp and port not 22", "-w",  directory + "traffic.pcap"});
        });
        delay(1000);
...

        {
            ...
            ChildProcess container_process( "recordshell", [&]() {
                    ...
                    // ChildProcess tcpdump("tcpdump", [&]() {
                    //     return ezexec({"/usr/sbin/tcpdump", "-i", ingress_name, "tcp", "-w", directory + "ingress.pcap"});
                    // });
                    // delay(1000);
