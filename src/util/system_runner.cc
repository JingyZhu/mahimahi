...
int ezexec( const vector< string > & command, const bool path_search, const bool redirect, string filename)
{
    if ( command.empty() ) {
...
void run( const vector< string > & command, const bool path_search )
{
    ChildProcess command_process( join( command ), [&] () {
            return ezexec( command, path_search);
        } );
